...
public class Task2 {
  ...
  public static List<Person> combineAndSortWithLimit(Collection<Person> persons1,
                                                     ...
                                                     int limit) {
    List<Person> limitPersonList = Stream
        .concat(persons1.stream(), persons2.stream())
        .sorted(Comparator.comparing(Person::createdAt))
        //.sorted((person1, person2) -> person1.createdAt().compareTo(person2.createdAt()) изначально хотел сделать так
        .limit(limit)
        .toList();

    return limitPersonList;
  }
}
